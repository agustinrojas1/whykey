...
| L-009 | Offline snapshots | Snapshots preserve a redacted diagnostic conclusion for replay and comparison, not the raw IPC/configuration inputs required to re-run every adapter offline. | `snapshot::create` redacts shell identity and private paths; README documents the boundary. |
...
| D-016 | Hyprland cleanup refuses a submap restore when the owner state is absent or belongs to another token, preventing a stale guard from changing a newer capture. | `cleanup_lua_code_refuses_to_restore_when_owner_state_is_missing`; `stale_tokens_cannot_disarm_another_session`. |
