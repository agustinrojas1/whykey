...
| L-003 | Keyboard layout | Evdev-to-XKB translation remains conditional when the compositor does not expose complete XKB type, latch, and lock state. Hyprland exposes pressed modifiers plus CapsLock/NumLock LEDs; Sway exposes neither latch nor lock state through its read-only IPC. | Shared `src/xkb.rs` parses real `symbols[N]` layout groups; `layers/hyprland/keyboard.rs` supplies typed runtime state; `XkbState::level()` preserves `NoSymbol` positions and keeps selection conditional when evidence is missing. Issue #74 records the accepted shift-only latch boundary. |
