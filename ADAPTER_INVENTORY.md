...
| KDE Plasma | KDE desktop identity or `kglobalshortcutsrc` path | `kglobalshortcutsrc` (or `KGLOBALSHORTCUTS_CONFIG`) plus `org.kde.KGlobalAccel` introspection | service reachability only | literal file bindings with live-vs-file provenance | live registrations, runtime reload, and plugin shortcuts remain conditional |
